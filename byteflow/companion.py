...
def run_companion(agent=None, model="llama3", enable_desktop_tools=True,
    ...
    def on_upload():
        ...
        try:
            ...
            append_message("System", f"[Could not read {filename}] {e}")
            return

        # Ingest the FULL extracted text into the vector store - chunked
        # automatically if it's long (see chunking.py) - rather than
        # truncating it or dumping the whole thing into this one
        # message. recalled_context() will pull in just the relevant
        # chunk(s) for whatever you ask about it next, even for files
        # far too big to fit in one prompt.
        #
        # read_file_text() detects the real file type (.pdf via pypdf,
        # .docx via python-docx, anything else as plain text) - this is
        # what actually fixes a real bug where PDFs/DOCX files were
        # being opened as plain UTF-8 text, producing meaningless binary
        # garbage that got indexed instead of the document's real content.
