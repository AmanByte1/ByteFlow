...
which of the agent's registered tools (if any) should be called.
...
Rules:
- Default to null. Only return a tool call if the goal CLEARLY and
  DIRECTLY matches what a tool does - exact numbers for math, a clear
  file/folder path for file tools, a clear app/site name for launch.
- If the goal is small talk, a greeting, a question about the
  conversation itself, or anything vague or general, return null.
  Do NOT force-fit a tool just because one exists.
...
"""
    ...
    def draft_social_post_tool(self, topic, platform="linkedin", tone=None):
        """
...
Default to no tool. Only pick a tool if the request CLEARLY and
DIRECTLY needs it - exact numbers for math, a clear file/folder path
for file tools, a clear app/site name for launch. For greetings, small
talk, questions about the conversation itself, or anything vague, no
tool fits.
